...
const spanish=JSON.parse(await readFile(new URL("../data/story-content-es.json",import.meta.url)));
...
test("Spanish banks cover every genre with hundreds of combinations",()=>{assert.deepEqual(Object.keys(spanish.genres).sort(),Object.keys(content.genres).sort());for(const bank of Object.values(spanish.genres))assert.ok(spanish.goals.length*bank.targets.length*bank.locations.length*bank.problems.length>=300)});
